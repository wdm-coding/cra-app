...
class HttpService {
  ...
  async get<T>(url: string, params?: any, config?: any): Promise<T> {
    const response = await axiosInstance.get<BaseResponse<T>>(url, { params, ...config });
  ...
  // DELETE请求
  async delete<T>(url: string, params?: any, config?: any): Promise<T> {
    const response = await axiosInstance.delete<BaseResponse<T>>(url, { params, ...config });
    return this.handleResponse(response);
  }
  // 处理响应数据
