...
import catalogBanner from '@/assets/images/portalImg/catalog-banner.png'
...
const Catalog = () => {
  ...
  const fetchTotalCount = useCallback(async () => {
    const { code, data } = await getResourceList({ pageNum: 1, pageSize: 1 })
    if (code === 200) {
  ...
  return (
    <div>
      ...
      >
        {(item) => (
          <div className={styles.pageListItemContent} key={item.catalogId}>
              ...
              <div className={styles.textSty}>
                ...
                <div className={styles.viewCount}>
                  ...
                  <span style={{ marginLeft: '0.4vw' }}>
                    {item?.catalogStatisticsVO?.viewCount || 0}次浏览
                  </span>
                </div>
              </div>
            </div>
            <div className={styles.infoContent}>
              <span>行业分类：</span>
              <span>{item?.dynamicCatalogFields?.industry || '-'}</span>
            </div>
            <div className={styles.infoContent}>
              <span>登记时间：</span>
              <span>{item?.dynamicCatalogFields?.catalogRegisterTime || '-'}</span>
            </div>
